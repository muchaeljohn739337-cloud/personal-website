 ...
 * Uses Vercel API token to set all required environment variables
 */
...
async function getEnvVarId(name: string): Promise<string | null> {
  try {
    const url = `${VERCEL_API_URL}/v10/projects/${VERCEL_PROJECT_ID}/env`;
    ...
    if (response.ok) {
      ...
      const existing = envVars.find((env: { key: string }) => env.key === name);
...
async function setVercelEnvVar(
  ...
  try {
    ...
    const existingId = await getEnvVarId(name);

    if (existingId) {
      // Update existing variable
      const url = `${VERCEL_API_URL}/v10/projects/${VERCEL_PROJECT_ID}/env/${existingId}`;
      const response = await fetch(url, {
        method: 'PATCH',
        headers: {
          Authorization: `Bearer ${VERCEL_TOKEN}`,
      ...
      if (response.ok) {
        ...
        console.log(`   Update error: ${error.substring(0, 100)}`);
        return false;
      }
    } else {
      // Create new variable
      ...
      if (response.ok) {
        ...
        const error = await response.text();
        console.log(`   Create error: ${error.substring(0, 100)}`);
        return false;
      }
    }
  } catch (error: unknown) {
...
async function main() {
  ...
  if (failCount > 0 && missing.length > 0) {
    console.log('💡 For missing variables, add them to .env.local first.\n');
  }
