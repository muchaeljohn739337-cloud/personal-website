...
$backendJob = Start-Job { Set-Location $using:backend; npm start }
$frontendJob = Start-Job { Set-Location $using:frontend; npm start }
...
Write-Host ""
Write-Host "💡 To stop servers, run: Get-Job | Stop-Job; Get-Job | Remove-Job" -ForegroundColor Yellow
